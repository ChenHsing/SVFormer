...
class UnlabelMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.net_timer = Timer()
        self.loss = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_label = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_unlabel = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_total = 0.0
        self.lr = None
        # Current minibatch errors (smoothed over a window).
        self.mb_top1_err = ScalarMeter(cfg.LOG_PERIOD)
    ...
    def reset(self):
        ...
        """
        self.loss.reset()
        self.loss_label.reset()
        self.loss_unlabel.reset()
        self.loss_total = 0.0
        self.lr = None
        self.mb_top1_err.reset()
        self.mb_top5_err.reset()
    ...
    def update_stats(self, top1_err, top5_err, loss,loss_label, loss_unlabel, lr, mb_size, stats={}):
        """
        Update the current stats.
        Args:
            top1_err (float): top1 error rate.
        ...
        """
        self.loss.add_value(loss)
        self.loss_label.add_value(loss_label)
        self.loss_unlabel.add_value(loss_unlabel)
        self.lr = lr
        self.loss_total += loss * mb_size
        self.num_samples += mb_size
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "eta": eta,
            "loss": self.loss.get_win_median(),
            "loss_label":self.loss_label.get_win_median(),
            "loss_unlabel": self.loss_unlabel.get_win_median(),
            "lr": self.lr,
            "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
        }
        if not self._cfg.DATA.MULTI_LABEL:
