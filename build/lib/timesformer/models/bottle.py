...
class Block(nn.Module):
    ...
    def forward(self, x, fast, idx, B, T, W):
        num_spatial_tokens = (x.size(1) - 1) // T
        H = num_spatial_tokens // W

        fast = fast[:,0:1,:]
        fast = self.linear(fast)
        x = torch.cat([fast,x],dim=1)
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            ## Spatial
            init_fast_token = x[:,0,:].unsqueeze(1)
            fast_token = init_fast_token.repeat(1, T, 1)
            fast_token = rearrange(fast_token, 'b t m -> (b t) m',b=B,t=T).unsqueeze(1)

            init_cls_token = x[:,1,:].unsqueeze(1)
            cls_token = init_cls_token.repeat(1, T, 1)
            cls_token = rearrange(cls_token, 'b t m -> (b t) m',b=B,t=T).unsqueeze(1)
            xs = xt
            xs = rearrange(xs, 'b (h w t) m -> (b t) (h w) m',b=B,h=H,w=W,t=T)
            xs = torch.cat((cls_token,fast_token, xs), 1)
...
class VisionTransformer_slow(nn.Module):
    """ Vision Transformere
    """
    ...
    def forward_features(self, x, lis):
        B = x.shape[0]
    ...
    def forward(self, x, lis):
        x_fea = self.forward_features(x,lis)
...
if __name__ == '__main__':
    ...
    model = VisionTransformer_slow(img_size=224, num_classes=400, patch_size=16,
                                        embed_dim=768,
                                        depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                                        norm_layer=partial(nn.LayerNorm, eps=1e-6),
                                        drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=8,
                                        attention_type=attention_type)
    lis = []
    for i in range(12):
        fast = torch.rand(2,3137,768)
        lis.append(fast)
    out = model(x,lis)
