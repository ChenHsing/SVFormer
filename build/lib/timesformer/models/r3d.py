...
class ResNet3d(nn.Module):
    ...
    @staticmethod
    def _inflate_weights(self, logger=None):
    ...
    def _init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            if self.pretrained2d:
                ...
                load_checkpoint(
                    self, self.pretrained, strict=False)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.out_indices:
                outs.append(x)
        if len(outs) == 1:
            out = self.avg_pool(outs[0]).squeeze(-1).squeeze(-1).squeeze(-1)
            out = self.head(out)
            return out
...
class ResNet3d18(nn.Module):
    def __init__(self, pretrained_model = '/mnt/blob/resnet18.pth',  **kwargs):
        super(ResNet3d18, self).__init__()
        self.model = ResNet3d(depth=18,
        pretrained='/mnt/blob/resnet18.pth',
        pretrained2d=True,
        norm_eval=False,
        conv_cfg=dict(type='Conv3d'),
        act_cfg=dict(type='ReLU'),
...
if __name__=='__main__':
    pretrained = 'C:/model/resnet18.pth'
    model = ResNet3d18(pretrained=pretrained)
